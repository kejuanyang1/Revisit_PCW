...
PRETRAINED_VOCAB_FILES_MAP = {
    ...
    "gpt-neo-1.3b": "/zhangpai22/kejuan/checkpoints/gpt-neo-1.3B",
    "gpt-neo-2.7b": "/zhangpai22/kejuan/checkpoints/gpt-neo-2.7B",
}
...
class HFGPT2Tokenizer(HFTokenizer):
    def __init__(self, model_type_or_path=None, cache_dir=None):
        super().__init__(GPT2Tokenizer, model_type_or_path=model_type_or_path, cache_dir=cache_dir)
        if "gpt" in model_type_or_path:
            command_tokens = [
                CommandToken('eos', '<|endoftext|>', self.TokenToId("<|endoftext|>")),
                CommandToken('pad', '<|endoftext|>', self.TokenToId("<|endoftext|>")),
                CommandToken('sop', '<|endoftext|>', self.TokenToId("<|endoftext|>")),
                CommandToken('eop', '<|endoftext|>', self.TokenToId("<|endoftext|>"))
            ]
        elif "opt" in model_type_or_path:
            command_tokens = [
                CommandToken('eos', '</s>', self.TokenToId("</s>")),
                CommandToken('pad', '<pad>', self.TokenToId("<pad>")),
                CommandToken('sop', '</s>', self.TokenToId("</s>")),
                CommandToken('eop', '</s>', self.TokenToId("</s>"))
            ]
        self.command_tokens = command_tokens
